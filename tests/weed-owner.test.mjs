...
test("city Toronto page is demoted to noindex with homepage canonical", () => {
  assert.match(route, /canonical: "https:\/\/www\.indigenousmidtowncannabis\.ca\/"/);
  assert.match(route, /index: false/);
  assert.match(route, /follow: true/);
});
test("owner uses exact verified identity and facts", () => {
...
test("approved discovery routes are present", () => {
  for (const href of ["/resources/weed-flower-guide", "/resources/menu-guide", "/resources/weed-value-guide", "/resources/pre-roll-guide", "/resources"])
    assert.ok(owner.includes(`"${href}"`));
  assert.doesNotMatch(owner, /available now|in stock|best price/i);
});
test("copy makes no unsupported cultural claim", () => {
  assert.match(owner, /No additional ownership, cultural or affiliation claim is made/);
